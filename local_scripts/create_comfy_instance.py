#!/usr/bin/env python3
"""
Create a new ThunderCompute instance tailored for ComfyUI with WAN and QWEN models.
...
def main():
    parser = argparse.ArgumentParser(description="Create and setup ComfyUI instance on ThunderCompute")
    ...
    parser.add_argument("--disk-size", type=int, default=300,
                      help="Disk size in GB (default: 300)")
    parser.add_argument("--setup-timeout", type=int, default=2400,
                      help="Timeout for full setup in seconds (default: 2400)")
    parser.add_argument("--skip-setup", action="store_true",
                      help="Only create instance, skip ComfyUI setup")
    ...
    try:
        with ThunderComputeManager.from_secrets() as manager:
            # Create instance with ComfyUI-optimized specs
            print("Creating ThunderCompute instance for ComfyUI...")
            ...
            if args.skip_setup:
                ...
                print(f"To setup ComfyUI later, run:")
                print(f"  python local_scripts/setup_comfy_instance_complete.py -i {instance_id}")
                return
            ...
            # Run complete ComfyUI setup
            print(f"\nStarting ComfyUI setup (timeout: {args.setup_timeout}s)...")
            print("This will install:")
            print("  - Miniforge conda distribution")
            print("  - ComfyUI environment and CLI")
            print("  - Qwen Image models for ComfyUI")
            ...
            # Check setup success
            if "Full setup complete" in output:
                print("✅ ComfyUI setup completed successfully!")
                print(f"\nInstance {instance_id} is ready for ComfyUI!")
                
                print("\nNext steps:")
                print(f"  1. Start ComfyUI: python local_scripts/start_comfy_with_tunnel.py -i {instance_id}")
                print(f"  2. Or SSH manually: tnr connect {instance_id}")
                print("  3. Then: conda activate comfy-env && comfy ui --port 8188")
