#!/usr/bin/env python3
"""
Copy all remote_scripts to a ThunderCompute instance and execute full_setup.sh.
This sets up the complete environment: Miniforge, ComfyUI, and downloads models.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run full setup on ThunderCompute instance")
    parser.add_argument("-i", "--instance-id", type=int, required=True,
                      help="ThunderCompute instance ID")
    parser.add_argument("--timeout", type=int, default=1800,
                      help="Timeout for setup in seconds (default: 1800)")
    parser.add_argument("-v", "--verbose", action="store_true",
                      help="Show detailed output")
    parser.add_argument("--session-name", default="full-setup",
                      help="Tmux session name (default: full-setup)")
    ...
    try:
        with ThunderComputeManager.from_secrets() as manager:
            ...
            print(f"Starting full setup in tmux session '{args.session_name}'...")
            print("This will install Miniforge, ComfyUI environment, and download models")
            print(f"Estimated time: 10-20 minutes (timeout: {args.timeout}s)")
            ...
            if "Full setup complete" in output:
                ...
                print("  ✓ ComfyUI environment (comfy-env)")
                print("  ✓ ComfyUI CLI tools")
                print("  ✓ Qwen Image models")
                
                print(f"\nTo use ComfyUI on instance {instance_id}:")
                print("  1. SSH to instance: tnr connect {instance_id}")
                print("  2. Activate environment: conda activate comfy-env")
                print("  3. Start ComfyUI: comfy ui --port 8188")
                print("  4. Access via tunnel or direct connection")
